...
export const POLICY_ID: Hex = env.VITE_POLICY_ID && /^0x[0-9a-fA-F]{64}$/.test(env.VITE_POLICY_ID) ? (env.VITE_POLICY_ID as Hex) : ZERO_HASH
...
/** Display labels for predicate bits. Meaning is per policy; this is the demo policy's labelling. */
export const BIT_LABELS: readonly string[] = ['identity evidence', 'over 18', 'EU resident', 'not sanctioned']
...
export const CONFIG_WARNINGS: string[] = MOCK
  ...
  : [
      ...
      POLICY_ID === ZERO_HASH ? 'VITE_POLICY_ID is not set' : '',
    ].filter(Boolean)
