 ...
 *
 * Bounds come from the artifact ABI (`CircuitBounds`); the JSON fragments the
 * circuit matches (`vct`, `age_equal_or_over._sd`, `cnf.jwk`, `exp`, `aud`,
 * `nonce`, `sd_hash`) and the 46-byte `_sd` entry stride are fixed by
 * `circuits/pid-sdjwt/src/constants.nr`. The `aud` value is a parameter: the
 * circuit pins one string, and the app must pass the same one.
 */
object ProverInputs {
    /** aud pinned by the current circuit (constants.nr AUD_FRAGMENT); the revised circuit pins the registered client_id. */
    const val DEFAULT_AUD = "https://self-issued.me/v2"
    ...
    fun derive(
        ...
        expectedAud: String = DEFAULT_AUD,
    ): Derived {
        ...
        if (sigB64.length != bounds.issuerSigB64Len) fail("issuer signature base64url length ${sigB64.length}, expected ${bounds.issuerSigB64Len}")
        ...
        if (ageDigestIndex >= bounds.maxAgeEntries) fail("age digest index exceeds MAX_AGE_ENTRIES")
        ...
        if (subject.size != bounds.subjectLen || challenge.size != bounds.challengeLen) fail("subject must be ${bounds.subjectLen} bytes, challenge ${bounds.challengeLen} bytes")
        ...
        sb.append(bounded("issuer_header_b64", headerB64.toByteArray(Charsets.US_ASCII), bounds.headerB64Max))
        sb.append(bounded("payload", payloadRaw, bounds.payloadMax))
        sb.append("issuer_sig_b64 = ${bytes(sigB64.toByteArray(Charsets.US_ASCII))}\n")
        sb.append("issuer_sig = ${bytes(lowS(issuerSigRaw))}\n")
        sb.append(bounded("disclosures_tail", tail.toByteArray(Charsets.US_ASCII), bounds.tailMax))
        sb.append(bounded("kb_payload", kbPayloadRaw, bounds.kbPayloadMax))
        sb.append("kb_signature = ${bytes(lowS(kbSig))}\n")
        sb.append("issuer_pub_x = ${bytes(issuerX)}\n")
        sb.append("issuer_pub_y = ${bytes(issuerY)}\n")
        sb.append(bounded("age_salt", ageSalt.toByteArray(Charsets.US_ASCII), bounds.saltMax))
