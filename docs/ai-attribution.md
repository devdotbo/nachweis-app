...
- Wiki repository: `devdotbo/nachweis`, private during the build and staying private (an exported session transcript is committed in its history). The spec-driven artifacts are therefore copied into this repository: `docs/wiki/README.md` gives the read order and names the wiki commit the copy was taken from. Read order for a judge: `AGENTS.md` (schema and writing rules), `index.md`, `wiki/decisions.md`, `wiki/work-packages.md` (acceptance test and evidence class per package), `log.md` (one dated line per turn).
