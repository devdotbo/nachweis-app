...
Fails, with the reason (CLAIM, from code; not exercised on the public page). State of the first build of 15:03 without relay and bridge; superseded the same afternoon by the rebuild with `VITE_VERIFIER_URL` and `VITE_BRIDGE_URL` (`docs/hosting-server.md`, "App deployment") and by the journey recorded in the last section of this file:
...
- The header stays: bb.js proves with a thread pool on `SharedArrayBuffer`, which needs `crossOriginIsolated`, and the browser proof is the demonstrated route. So the public app was rebuilt without the app id (same exports as the recorded rebuild; `vercel rollback` to `attestat-9iob7erx5-...` failed with "Deployment belongs to a different team" from the CLI, so a fresh prebuilt deploy replaced it) and no Privy option is offered on https://app.attestat.dev; headless Chromium on the investor page renders "Connect MetaMask/injected wallet" and no Privy option, loads no iframe and nothing from auth.privy.io (the only asset name matching "privy" is the app's own `privyContext` chunk (the context object, not the SDK); the label string in wallet.ts is static and gated at runtime), `crossOriginIsolated` is true, and the COOP and COEP headers are served unchanged. Serving deployment: `attestat-qvcthd8c7-...`. The `showWalletUIs: false` setting stays in the code for a Privy build on an origin without COEP (local `bun run dev` with the app id, or a later hosting shape).
...
## The journey on the public app (2026-09-13, afternoon and evening)

- FACT: since the rebuild with the relay and the bridge on the builder's server (`docs/hosting-server.md`), https://app.attestat.dev runs the journey for a visitor with an injected wallet on Sepolia: session at bridge.attestat.dev, presentation request through relay.attestat.dev, scan with the official test wallet on the phone, proof in the browser tab, `attestWithProof` sent by the hosted bridge, then the two doors. On 2026-09-13 the relay stored four wallet responses and the bridge mined three attestations by browser proof for three addresses; the operator's own address (approved since the deployment) then subscribed and swapped twice through the page; two addresses without approval got the pool's refusal, mined with status 0 and decoded to `Unauthorized()` in `PermissionedHooks.beforeSwap`. The record with every hash is `docs/evidence/sepolia-public-2026-09-13.md`; the serving deployment is `attestat-qvcthd8c7-...` and its bundle is byte-identical to a local build of commit `a1f9cd2` with the exports of the "App deployment" section.
- FACT: the bridge's operator balance ran dry at 15:19 UTC (a funding transfer from the operator address drained it) and one `noir-proof` request was refused with `-32003 insufficient funds` at 15:25 UTC; nothing was written on chain by that request, the operator was refilled at 15:26 UTC and the next proof was mined at 15:32 UTC. Keep the operator funded; the bridge has no balance alarm.
- What still does not work on the public page: the issuer approval for a visitor's address. The issuer console signs with the connected wallet and the registry accepts `approve` only from an operator; the bridge's token route needs `BRIDGE_ISSUER_TOKEN`, which the page does not carry. For the take address the approval was sent from a shell with the deployer key after the deadline (record above). The options are in `docs/handoff-2026-09-13-post-hackathon.md`, section 6.
