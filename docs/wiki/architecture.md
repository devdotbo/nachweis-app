...
One decision object, three consumers, one pluggable proof interface. Code: nachweis-app (contracts, service, app, circuits, prover-sp1) and the verifier worktree klartext-verifier (branch nachweis-relay, local checkout) (branch nachweis-relay). Background memo: raw/2026-09-06-pluggable-architecture.md.

## Input: EUDI presentation to verifier-service

- The investor's official test wallet answers an OpenID4VP request (DCQL, SD-JWT PID, vct urn:eudi:pid:de:1) from verifier-service. Registered minimal request: given_name, family_name, age_equal_or_over.18 (FACT, klartext-verifier/verifier-core/src/pid.rs:140-146).
...
- Address binding (new, WP3): the investor's wallet signs the EIP-191 session message; the KB-JWT nonce is lowercase hex of sha256(address20 || challenge32) (FACT, nachweis-app/prover-sp1/NOTES.md, statement item 6). The same definition binds the presentation to the address in the SP1 guest and in the relay branch. The EUDI wallet does not sign the address; the EVM key does (open question b).
...
FACT, nachweis-app/contracts/src/interfaces/IEligibility.sol:
...
FACT, nachweis-app/contracts/src/interfaces/IProofVerifier.sol and AttestationRegistry.attestWithProof:
...
The chain side is final in all three; only the prover location moves. The landing page states the same three steps (nachweis-site/index.html, section "Where the proof lives").
