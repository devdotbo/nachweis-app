...
- The Swap door in the investor portal (2026-09-09, `docs/swap.md`): `app/src/components/swap/calldata.ts` encodes the same `V4_SWAP` calldata with viem (`swapCalldata` at lines 36 to 75), quotes the exact-input amount from `StateView.getSlot0` and `getLiquidity` (`quoteExactIn` at 85 to 95; the deployed V4Quoter cannot quote a permissioned pool, `FEEDBACK.md` item 18) and decodes a refusal into words (`explainRevert` at 129 to 189: `WrappedError`, `Unauthorized`, `SwappingDisabled`, `HookNotAllowed`, `NotEligible`, `ExecutionFailed`); `app/src/components/swap/useSwap.ts` reads the pool and the adapter (`usePoolState` at 46 to 110), the three answers the pool relies on (`useSwapCheck` at 112 to 130: `registry.isEligible`, `checker.checkAllowlist`, `adapter.isAllowed`) and sends the swap from the connected wallet (`useSwapTx` at 167 to 260: faucet, ERC-20 approve to Permit2, Permit2 allowance, `eth_call` preflight, `UniversalRouter.execute`; a refused swap is sent with a fixed gas limit so it is mined and has a hash); `SwapDoor.tsx` is the tile, `SwapExplainer.tsx` the pool panel. Local pool: `scripts/pool-local.sh` (anvil fork of Sepolia with chain id 31337, the three scripts, a probe swap and a refused swap, `POOL-LOCAL PASS` in 24 s); browser run: `scripts/app-e2e-local.sh --mode sp1-mock --pool --test` with `app/e2e/swap.spec.ts` (swap, revoke in the console, refused swap decoded on screen). Screenshots in `docs/ui/` (`investor-11` to `investor-13`, `issuer-07`).
