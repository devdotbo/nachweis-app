...
- Deployment status: deployed on Sepolia on 2026-09-10 (section "Sepolia deployment (2026-09-10)" below, record `docs/deployments/sepolia-2026-09-10.md`, source-verified on Etherscan since 2026-09-13); the investor journey with the official wallet ran on that deployment on 2026-09-12 (record `docs/evidence/sepolia-phone-2026-09-12.md`, `attestWithProof` from a browser-made proof, then approve, subscribe, swap, revoke, refused swap). Before 2026-09-10 every Sepolia interaction was fork tests and dry runs; the SP1 verifier route stays on the fork. The hosted app https://app.attestat.dev (Vercel static build, relay and bridge on the builder's server, `docs/hosting-server.md`) ran the journey on 2026-09-13 with an injected browser wallet: three attestations by browser proof, subscribe and two swaps on two addresses, two refused swaps without approval (record `docs/evidence/sepolia-public-2026-09-13.md`).
...
- Verified on Etherscan (2026-09-13): all eight contracts above are source-verified (`forge verify-contract --chain sepolia --compilation-profile default|honk` with explicit constructor args; HonkVerifier linked against ZKTranscriptLib; details in the deployment record, section "Etherscan source verification (2026-09-13)"). The deployment itself ran without `ETHERSCAN_API_KEY` on 2026-09-10. `Sp1PidVerifier` is not deployed on Sepolia.
...
Evidence records (sanitized, `docs/evidence/`, index in [`docs/evidence/README.md`](docs/evidence/README.md)). Hosted-app record: [`docs/evidence/sepolia-public-2026-09-13.md`](docs/evidence/sepolia-public-2026-09-13.md), the journey on https://app.attestat.dev with an injected wallet and the official wallet on the phone (three addresses, 35 transactions re-read by receipt, the recorded take among them; the issuer approval for the take address was sent from a shell after the deadline, not from the hosted console). Primary journey record: [`docs/evidence/sepolia-phone-2026-09-12.md`](docs/evidence/sepolia-phone-2026-09-12.md), the official wallet on the builder's iPhone, proof made in the browser tab, separate issuer approval, subscribe, swap, revoke and the refused swap, all on Sepolia. Earlier records: [`docs/evidence/sepolia-journey-2026-09-10.md`](docs/evidence/sepolia-journey-2026-09-10.md) (the same journey on Sepolia without the phone, attested by the operator key) and [`docs/evidence/browser-real-wallet-2026-09-08.md`](docs/evidence/browser-real-wallet-2026-09-08.md) (official wallet against a local anvil, proof in the browser tab; the companion-prover run of the same day is `docs/evidence/g0-2026-09-08.md`).
...
Hosted: https://app.attestat.dev (Vercel, `docs/hosting-app-vercel.md`) with the relay and the bridge at https://relay.attestat.dev and https://bridge.attestat.dev (`docs/hosting-server.md`). The journey ran there on 2026-09-13 (`docs/evidence/sepolia-public-2026-09-13.md`). The approve step needs the operator wallet in the issuer console or the bridge's token route; a visitor's wallet stops at "evidence on chain, awaiting issuer approval" and the doors show the pool's refusal.
